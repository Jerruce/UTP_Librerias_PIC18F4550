...
int main(void) 
{
    ...
    while(1)
    {
        /* Realizar conversión con el ADC, calcular el voltaje medido 
         * y transmitir por el UART cada 1 segundo */
        if(INTCONbits.TMR0IF == 1)
        {
            ...
            sprintf(uart_buffer,"%.2f", voltaje);
            
            UARTn_Tx_String(UART0,"Voltaje: ");
            UARTn_Tx_String(UART0,uart_buffer);
